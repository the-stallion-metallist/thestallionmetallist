...
import { AddAdvance, EditExpense } from "../forms2";
...
export default function Staff() {
    ...
    return (<>
      ...
      <div className="card"><div className="card-h"><h2>{monthLabel(m)}</h2><span className="hint">Tap any past day to change it: present, half day, absent, clear</span><span className="sp" /><button className="btn btn-g btn-sm" onClick={addStaff}>+ Add staff member</button></div>
        ...
        <div className="tbl-wrap"><div className="cal" style={{ gridTemplateColumns: `110px repeat(${n},minmax(18px,1fr))` }}><span />
          ...
          {staff.map((s) => [<span key={"n" + s.id} className="nm">{s.name}</span>, ...days.map((x) => x > c.today ? <span key={x + s.id} className="c fut" />
  ...
  return (<>
    ...
    <div className="card"><div className="card-h"><div className="daynav"><button className="btn btn-g btn-sm" aria-label="Previous month" disabled={mo <= first} onClick={() => setMo(shiftMonth(mo, -1))}>‹</button><h2>{monthLabel(mo)} salary</h2>
      <button className="btn btn-g btn-sm" aria-label="Next month" disabled={mo >= last} onClick={() => setMo(shiftMonth(mo, 1))}>›</button></div><span className="sp" /><button className="btn btn-g btn-sm" onClick={() => c.openModal(<AddAdvance />)}>+ Record advance</button></div>
      ...
      <div className="tbl-wrap"><table><thead><tr><th>Staff</th><th className="n">Monthly salary</th><th className="n">Days present</th><th className="n">Earned</th><th className="n">Advances</th><th className="n">Paid so far</th><th className="n">To pay</th><th /></tr></thead><tbody>
        {staff.map((s) => { const d = staffDue(s, mo, c.marks, c.advances, c.expenses);
          return <tr key={s.id}><td><b>{s.name}</b></td><td className="n"><input className="inl" type="number" inputMode="numeric" min={0} defaultValue={s.salary ?? ""} placeholder="Set ₹" aria-label={`${s.name} monthly salary`} onBlur={(e) => setSal(s, e.target.value)} /></td>
      ...
      {advs.length ? <div className="kv" style={{ marginTop: 14 }}>{advs.map((a) => <div key={a.id}><span>{dnice(a.d)} · {c.staff.find((s) => s.id === a.staff_id)?.name} · advance</span><b>{rs(Number(a.amount))}</b></div>)}</div>
