...
import { restorePickup } from "../forms";
  ...
  return (
    <div className="card">
      ...
      {L.length ? L.map((x) => {
        ...
        const pool: Record<string, { id: number; deleted: boolean }[]> = { pickups: c.pickups, payments: c.payments, expenses: c.expenses, sales: c.sales, trips: c.trips, advances: c.advances };
        const canRestore = x.action === "Deleted" && x.ref_table && pool[x.ref_table]?.find((p) => p.id === x.ref_id)?.deleted;
        return <div key={x.id} className="hist"><span className="av">{x.who[0]}</span>
          ...
          {canRestore && <button className="btn btn-g btn-sm" onClick={() => (x.ref_table === "pickups" ? restorePickup(c, x.ref_id!) : restoreRow(c, x.ref_table!, x.ref_id!))}>Restore</button>}</div>;
