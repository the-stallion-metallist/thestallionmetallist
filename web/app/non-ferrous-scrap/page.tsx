  ...
  return (
    <SiteFrame>
      ...
      <style>{`
        ...
        .nf-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 1.2rem; }
        .nf-card { display: flex; flex-direction: column; text-decoration: none; overflow: hidden;
          background: var(--card); border: 1px solid var(--line); border-radius: var(--r-lg);
          color: var(--ink); transition: border-color .25s ease, transform .25s ease, box-shadow .25s ease; }
        .nf-card:hover { border-color: color-mix(in srgb, var(--copper) 55%, var(--line));
          transform: translateY(-4px); box-shadow: 0 22px 48px -30px rgba(0,0,0,0.45); }
        /* Fixed, compact landscape image area (prevents the grid from stretching it). */
        .nf-thumb { width: 100%; height: 190px; flex: none; object-fit: cover; display: block;
          transition: transform .4s ease; }
        .nf-card:hover .nf-thumb { transform: scale(1.05); }
        .nf-ph { display: flex; align-items: center; justify-content: center;
          ...
          color: var(--copper-dk); font-family: var(--f-disp); font-weight: 700; font-size: 1.35rem; letter-spacing: -0.02em; }
        .nf-body { display: flex; flex-direction: column; gap: 0.5rem; padding: 1.3rem 1.5rem 1.5rem; flex: 1; }
        .nf-metal { font-family: var(--f-disp); font-weight: 700; font-size: 1.35rem; letter-spacing: -0.02em; }
        .nf-body p { color: var(--muted); font-size: 0.92rem; line-height: 1.5; flex: 1; }
        .nf-go { color: var(--copper); font-weight: 600; font-size: 0.88rem; margin-top: 0.3rem; }
        @media (prefers-reduced-motion: reduce){ .nf-card:hover, .nf-card:hover .nf-thumb { transform: none; } }
